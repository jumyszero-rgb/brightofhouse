...
export default function AdminServicesPage() {
  ...
  const addDetail = async (itemId: string, currentDetails: Detail[]) => {
    ...
    setNewDetail({ ...initialDetail, itemId: "" });
    await fetchData();
    setLoading(false);
  };
              ...
              <div className="bg-slate-100 p-4 flex justify-between items-center border-b border-slate-200">
                {editingId === cat.id ? (
                  <div className="flex gap-2 flex-1">
                    <input className="flex-1 p-1 border rounded text-black" value={editData.title} onChange={(e) => setEditData({...editData, title: e.target.value})} />
                    <button onClick={saveEdit} className="bg-green-600 text-white px-3 rounded text-sm">保存</button>
                    <button onClick={() => setEditingId(null)} className="bg-gray-400 text-white px-3 rounded text-sm">中止</button>
                  </div>
                ) : (
                  ...
                  <div key={item.id} className="border border-slate-200 rounded-lg p-4">
                            ...
                            {editingId === detail.id ? (
                              <td colSpan={4} className="py-2 bg-yellow-50 p-2 rounded">
                                <div className="flex flex-col gap-2">
                                  <div className="flex gap-2">
                                    <input className="w-1/3 p-1 border rounded text-black text-xs" value={editData.label || ""} onChange={(e) => setEditData({...editData, label: e.target.value})} placeholder="ラベル" />
                                    <input className="flex-1 p-1 border rounded text-black text-xs" value={editData.value} onChange={(e) => setEditData({...editData, value: e.target.value})} placeholder="値" />
                                  </div>
                    ...
                    <div className="bg-gray-50 p-2 rounded">
                      ...
                      <div className="flex gap-4 items-center flex-wrap">
                        ...
                        <button onClick={() => addDetail(item.id, item.details)} className="bg-slate-600 text-white px-3 py-1 rounded text-xs ml-auto">追加</button>
                      </div>
                    </div>
                  </div>
                ))}
